...
const JobPost: NextPage = () => {
  ...
  const [COMPANY_LOGO_PRICE, setCompanyLogoPrice] = React.useState(0);
  const [CUSTOM_BRAND_COLOR_PRICE, setCompanyColorPrice] = React.useState(0);
  const [STICKY_POST_PRICE_24H, setSticky24hPrice] = React.useState(0);
  const [STICKY_POST_PRICE_7D, setSticky7dPrice] = React.useState(0);
  const [STICKY_POST_PRICE_14D, setSticky14dPrice] = React.useState(0);
  const [STICKY_POST_PRICE_30D, setSticky30dPrice] = React.useState(0);
  const [JOB_PRICE, setJobPrice] = React.useState(0);
...
const getProductList = async() => {
  const res = await axios.get('/api/getProductList');
  const list = res.data.list;
  formik.setFieldValue("price", list?.jobPost.price);
  setJobPrice(list?.jobPost.price);
  formik.setFieldValue("list",[{price: list?.jobPost.id, quantity: 1}]);
  setProductList(list);
  setCompanyLogoPrice(list?.logo.price);
   setCompanyColorPrice(list?.brandColor.price);
   setSticky24hPrice(list?.sticky_24h.price);
   setSticky7dPrice(list?.sticky_7d.price);
   setSticky14dPrice(list?.sticky_14d.price);
   setSticky30dPrice(list?.sticky_30d.price);
}
useEffect(() => {
  getProductList();
},[])
...
},[formik.values.images, formik.values.customBrandColor, formik.values.sticky,productList]);

const calculateNewPrice = () => {

  const list = [{price: productList?.jobPost.id, quantity: 1}];
  let newPrice = JOB_PRICE;
  if( formik.values.images[0]){
    newPrice += COMPANY_LOGO_PRICE;
    list.push({price: productList?.logo.id, quantity: 1});
  }
  if( formik.values.customBrandColor){
    newPrice += CUSTOM_BRAND_COLOR_PRICE;
    list.push({price: productList?.brandColor.id, quantity: 1});
  }
  if( formik.values.sticky === 24){
    newPrice += STICKY_POST_PRICE_24H;
    list.push({price: productList?.sticky_24h.id, quantity: 1});
  }
  if( formik.values.sticky === 168){
    newPrice += STICKY_POST_PRICE_7D;
    list.push({price: productList?.sticky_7d.id, quantity: 1});
  }
  if( formik.values.sticky === 336){
    newPrice += STICKY_POST_PRICE_14D;
    list.push({price: productList?.sticky_14d.id, quantity: 1});
  }
  if( formik.values.sticky === 720){
    newPrice += STICKY_POST_PRICE_30D;
    list.push({price: productList?.sticky_30d.id, quantity: 1});
  }
            ...
            <div className={stylesJobDescription.previewPostContainer}>
              ...
              onClick= { async() => {
                ...
                if(key){
                  ...
                  const response = await axios.post('/api/jobPost', formik.values);
              ...
              >
                Start Hiring! - ${formik.values.price}
              </div>
            </div>
          </div>
          <div
          ...
          >
            ...
            >
              ...
              >
                ...
                <div>
                  <h4 className={stylesJobDescription.jobPostInputTitle}>
                    Company Logo (.JPG or .PNG) (+${COMPANY_LOGO_PRICE})
                  </h4>
                  <ImageUploading
                    multiple
                ...
                <div style={{ marginBottom: "75px" }}>
                  <h4
                    ...
                    style={{ display: "flex", placeItems: "center" }}
                  >
                    Custom brand color +${CUSTOM_BRAND_COLOR_PRICE}:
                    <label className={stylesJobPost.container}>
                      <input
                        type="checkbox"
                        name="checkbox"
              ...
              >
                ...
                <div
                  className={[
                    stylesJobPost.separatorCompany,
                    stylesJobPost.separatorCompanyLast,
                  ].join(" ")}
                >
                  <h4
                    className={stylesJobDescription.jobPostInputTitle}
